...
#define HUNTER_CHEAT // "HELLYEAH" spawns a Hunter
...
#define CUSTOM_WEB_RADIO // CUSTOM (fork): adds the "80s80s Rock" internet-radio station. Streams MP3 over
                         // HTTP(S) and falls back to static noise when unreachable. Real audio needs the OAL
                         // backend with AUDIO_OAL_USE_MPG123; networking uses WinHTTP on Windows (static-only elsewhere).
